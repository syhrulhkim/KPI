                                    ...
                                    <div class="col-md-4 mt-2" id="policyupload">
                                        <div class="form-group">
                                            ...
                                            <div wire:loading wire:target="policy_path"><i class="mdi mdi-loading mdi-spin mdi-24px"></i></div>
                                                <input type="file" wire:model="policy_path" id="policy_path" name="policy_path" class="dropify" required />
                        ...
                        <tbody>
                          ...
                          @foreach ($policy as $key => $policys)
                            <tr>
                              ...
                              <td>
                                <p class="text-xs font-weight-bold mb-0" value="{{ $policys->title }}">{{ $policys->title }}</p>
                              </td>
                              <td>
                                <p class="text-xs font-weight-bold mb-0" value="{{ $policys->user->name }}">{{  $policys->user->name }}</p>
                              </td>
                              <td>
                                <p class="text-xs font-weight-bold mb-0" value="{{ $policys->user->position }}">{{  $policys->user->position }}</p>
                              </td>
                              <td class="text-xs font-weight-bold mb-0">{{date('j F Y', strtotime($policys->updated_at))}} </td>
                              <td class="align-middle text-center">
                                <a href=" {{ $policys->policy_path }}" style="color:blue;text-decoration:underline;font-size:13.5px"; target="_blank">View</a>
                              </td>
                              @if (auth()->user()->role == 'hr' || auth()->user()->role == 'admin')
                                ...
                                <div class="col-lg-6 col-5 my-auto text-middle">
                                  <div class="dropdown float-lg-start pe-4">
                                    ...
                                    <ul class="dropdown-menu px-2 py-3 ms-n4 ms-n5" aria-labelledby="dropdownTable">
                                      <li><a href="{{ url('hr/edit/policy/'.$policys->id) }}" class="dropdown-item border-radius-md" role="button">Edit</a></li>
                                      <li><button type="button" wire:click="selectItem({{$policys->id}})" class="dropdown-item border-radius-md data-delete" style="color: red;" data-form="{{$policys->id}}">Delete</button></li>
                                    </ul>
                                  </div>
                                </div>
                              </td>
