...
class KecekapanManager extends Component
{
    ...
    public function kecekapan_save(Request $request){
        ...
        public function kecekapan_update(Request $request,$id_user, $id) {
            ...
            // dd($id_user);
            if (KPIAll_::where('user_id', '=', $id_user)->count() == 1) {
                $kpiall = KPIAll_::where('user_id', '=', $id_user)->get();
                $kpiall_id = count($kpiall) > 0 ? $kpiall->sortByDesc('created_at')->first()->id : '0';
    
                $total_score_kecekapan = Kecekapan_::where('user_id', '=', $id_user)->sum('skor_sebenar');
                $total_score_nilai = Nilai_::where('user_id', '=', $id_user)->sum('skor_sebenar');
                $total_score_kpi = KPIMaster_::where('user_id', '=', $id_user)->sum('skor_sebenar');
                $total_score_all = ($total_score_kecekapan*0.1) + (($total_score_nilai/1.2)*0.1) + ($total_score_kpi*0.8);

                $weightage = Kecekapan_::where('user_id', '=', $id_user)->sum('peratus');
